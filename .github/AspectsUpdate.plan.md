...
- Overall app architecture (Bot + API + Mini App + PostgreSQL).
...
- **`AspectDefinitionModel`** (`aspect_definitions`): `id` (PK auto), `set_id` (bigint), `season_id` (int), `name` (text), `rarity` (text), `created_at` (datetime). Uses a **composite foreign key** via `ForeignKeyConstraint(["set_id", "season_id"], ["sets.id", "sets.season_id"], name="fk_aspect_definitions_set_season")` — mirroring the existing `ModifierModel` FK pattern, since `SetModel` has a composite PK `(id, season_id)`. This replaces the old modifier catalog for all new aspect-related code. Step 1 seeds/backfills it from the existing modifier dataset.
...
- **`AspectImageModel`** (`aspect_images`): `aspect_id` (PK, FK → owned_aspects.id, ondelete CASCADE), `image` (LargeBinary), `thumbnail` (LargeBinary), `image_updated_at` (nullable datetime). Mirrors the `card_images` table pattern — keeps metadata queries fast.
...
- **`RolledAspectModel`** (`rolled_aspects`): Mirrors `RolledCardModel` but tracks aspect rolls. Columns: `roll_id` (PK auto), `original_aspect_id` (BigInteger, not null, **unique=True**, no FK constraint), `rerolled_aspect_id` (nullable BigInteger, no FK constraint), `created_at` (datetime), `original_roller_id` (bigint), `rerolled` (bool, default False), `being_rerolled` (bool, default False), `attempted_by` (nullable text), `is_locked` (bool, default False), `original_rarity` (nullable text). **Note:** `original_aspect_id` and `rerolled_aspect_id` are intentionally plain BigInteger columns without FK constraints, matching `RolledCardModel`'s pattern for `original_card_id` / `rerolled_card_id`. This avoids cascade/constraint violations when aspects are burned or hard-deleted while historical roll records remain.
...
- **Critical:** The current `RolledCardManager._try_claim()` performs balance checks, claim-point deduction, and ownership assignment across **separate** `get_session()` calls (separate transactions). The generalized `RollManager` (Step 4) must **not** replicate this pattern. Instead, `RollManager._try_claim()` must delegate entirely to the new atomic `card_service.try_claim_card()` or `aspect_service.try_claim_aspect()`, which handle row-locking + balance deduction + ownership assignment in a single transaction. The manager's role is limited to pre-flight UI checks and post-claim UI updates — all DB mutation happens in the service layer.
...
- Reroll for aspects follows the same downgrade logic as card rerolls: rerolling a Rare aspect produces a Common aspect with a new random aspect definition and new sphere generation. Specifically: the original `OwnedAspectModel` + `AspectImageModel` are hard-deleted, a new aspect is created at the downgraded rarity with a fresh random aspect definition and new sphere image generation via `generate_aspect_image()`, and `RolledAspectModel.rerolled_aspect_id` is set to the new aspect's ID. This mirrors the card reroll flow exactly (which deletes the original card and creates a new one).
...
**Claim countdown / reveal UI:** The countdown background task is updated in the same phase to use `RollManager`, not a card-only manager, so it can reveal buttons and captions for either roll type. This includes `process_claim_countdown()` in `bot/api/background_tasks.py`, which currently imports `RolledCardManager` directly — this import and all usage within the function must be updated to use the generalized `RollManager`.
...
4. Update card image in `card_images` table, regenerate thumbnail, clear `file_id`. If image generation in step 3 fails, the card retains its old image but the aspect is still equipped and the modifier is updated — the card remains functional with stale art. Log a warning for manual resolution.
5. Send result card photo to chat with updated caption. If image generation failed, send a **distinct text-only notification** that clearly communicates: "Aspect equipped successfully, but image generation failed. Use /refresh to regenerate the card art." This must be clearly distinguishable from a full equip failure (which would have rolled back in step 2).
...
- **Update `COLLECTION_CAPTION`** in `bot/settings/constants.py`: the current template includes `{set_name}`, which will be `None` for all new cards (since cards no longer belong to sets). Update the template to conditionally display set info only when `set_name` is present (for legacy cards), and for new cards show equipped aspect count instead (e.g., "Aspects: 3/5"). Update all call sites in `collection.py` and the Mini App card detail views accordingly.
...
- **Callback data prefixes for aspect trades must be distinct** from card trades to avoid parsing ambiguity. Card trades continue using `trade_accept_{id1}_{id2}` / `trade_reject_{id1}_{id2}`. Aspect trades use new prefixes: `atrade_accept_{id1}_{id2}` / `atrade_reject_{id1}_{id2}`. Register separate callback handlers for each prefix set — matching the approach used for roll callbacks (`aclaim_`, `alock_`, `areroll_`). The existing `accept_trade` / `reject_trade` handlers parse callback data with `query.data.split("_")`, which would be ambiguous without distinct prefixes.
...
- Rename column `modifier` (name string) → `aspect_name` (Alembic `ALTER COLUMN ... RENAME TO`). Rename `modifier_id` → `aspect_definition_id`.
- **FK re-pointing and backfill:** The current `modifier_counts.modifier_id` has a FK constraint to `modifiers.id`. The Alembic migration must:
  1. Drop the existing FK constraint `ForeignKey("modifiers.id")` on `modifier_counts.modifier_id`.
  2. Backfill `aspect_definition_id` values by joining the renamed table on `modifiers.id` → matching `aspect_definitions` rows by `(name, set_id, season_id)` to map old modifier IDs to new aspect definition IDs (seeded in Step 1). Without this backfill, the `1/(1+count)` weighting system loses all historical frequency data and would bias toward already-common aspects.
  3. Create a new FK constraint pointing `aspect_definition_id` → `aspect_definitions.id`.
...
- **FK constraint cleanup (prerequisite to table drop):** Before the `modifiers` table can be dropped, all FK constraints pointing to it must be removed:
  1. Drop the FK constraint on `cards.modifier_id` → `modifiers.id` (the column itself can optionally be dropped entirely since new cards never set it, but keeping it as a plain nullable BigInteger for legacy data is also acceptable).
  2. Verify no other FK references remain (the `modifier_counts.modifier_id` FK was already dropped during the rename above).
  3. Only then execute `DROP TABLE modifiers`.
- Delete the remaining legacy `ModifierModel` ORM class, `Modifier` Pydantic DTO, `modifier_service` module, and related admin/frontend artifacts once every consumer has moved to `aspect_definitions`.
- Rename any lingering references to their aspect-definition equivalents.
